...
Existing approaches for generating toolpaths with adaptive width result in a large variation in width, which is beyond the capabilities of some hardware systems and difficult to manufacture accurately for others.
Existing approaches for generating toolpath with adaptive width targeted mostly at wire and arc additive manufacturing, and resulted in a large variation in width which is not typically supported by FDM printing.
%address this issue by making local alterations to the toolpaths locations and widths in the center of the part.
%In this paper we present a framework which allows to distribute the alterations from the center to the boundary.
In this paper we present a framework which supports multiple schemes to generate toolpaths with adaptive width, by using a function to decide the number of beads and their widths from the center outward.
