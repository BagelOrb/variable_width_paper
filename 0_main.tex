...
By densely filling consecutive 2D layers with contour-following extrusion toolpaths, FDM can produce parts with high stiffness,
which is especially important for thin parts such as blow molding prototypes and microstructures.
%
Existing approaches using uniform inward offsets from the outline polygons produce over- and underfill regions in the center of the shape.
Some existing approaches address this issue by making local alterations to the toolpaths locations and widths in the center.
Our framework makes it possible to distribute the workload of the alterations over larger regions farther from the center.
...
By carefully formulating various beading schemes for our framework we can emulate existing approaches and we propose a novel approach which incluences a larger region of toolpaths so that the toolpaths are altered less severely.
%
This approach reduces the amount of over- and underfill while the extrusion widths deviate less from the nozzle size compared to existing approaches.
\end{abstract}
