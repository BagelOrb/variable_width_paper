...
% My TK added packages and commands
    ...
    \usepackage{censor}
	\def\censorcolor{black}
	\let\svcensorrule\censorrule
	\renewcommand\censorrule[1]{%
	\textcolor{\censorcolor}{\svcensorrule{#1}}}
	
    \newcommand{\censoromit}[1]{}
	\newcommand{\censornotice}{}
	\ifcensor
		\renewcommand{\censornotice}{\\Anonymized 2020 (under revision)}
	\else
		\renewcommand{\censoromit}[1]{#1}
		\StopCensoring
    \fi
...
\title{A framework for adaptive width control of dense contour-parallel toolpaths in {fused deposition modeling}\censornotice}

%\author{Paper ID: xxx}

\author[um,tud]{\censor{Tim Kuipers}}
\author[tud]{\censor{Eugeni L. Doubrovski}}
\author[tud]{\censor{Jun Wu}\corref{cor1}}
\censoromit{\ead{j.wu-1@tudelft.nl}}
\cortext[cor1]{Corresponding author}
\author[cuhk]{\censor{Charlie Wang}}
% \ead{cwang@mae.cuhk.edu.hk}
\address[um]{\censor{Ultimaker, Utrecht, The Netherlands}}
\address[tud]{\censor{Department of Sustainable Design Engineering, Delft University of Technology, The Netherlands}}
\address[cuhk]{\censor{Department of Mechanical and Automation Engineering, The Chinese University of Hong Kong, Hong Kong SAR, China}}
...
{Toolpaths consisting of \emph{uniform}} inward offsets from the outline polygons produce over- and underfill regions in the center of the shape, which are especially {detrimental to the mechanical performance of} thin parts such as casings and microstructures.
...
Furthermore, we propose a novel scheme for FDM printing which {reduces} extreme bead width deviation, {while limiting} the number of altered toolpaths.
We validate the effectiveness of our framework and this novel scheme on a data set of 300 {layer outlines}.
We {statistically }validate the effectiveness of our framework and this novel scheme on a data set of {representative 3D models, }%sgd
{and physically validate it by developing a technique, called \emph{back pressure compensation}, for off-the-shelf FDM systems to effectively realize adaptive width.}
\end{abstract}
