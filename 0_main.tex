...
A simple technique for densely filling the outlines of a layer determines the toolpaths by performing inward offsets from the outline shape of a uniform width.
In the center of the outline shapes this simple technique can result in underfill regions and overfill regions as wide as the nozzle size.
...
We propose a framework which allows to define beading schemes which distribute the overfill and underfill required in each region over nearby toolpaths, so that we considerably reduce the amount of extrusion toolpaths with extreme widths.

Based on a skeletonization of the 2D outline shape of a single layer we generate a 3D surface mesh, the union of cones.
By slicing the surface at regular heights we obtain contour parallel toolpaths which cover the 2D area of a single layer.
The heights of the 3D mesh are first altered:
the heights at central ridges in the 3D surface are quantized in order to prevent over- and underfill
and the heights in other regions are modified in such a way that the deviation from the nominal extrusion width is distributed over several beads around the center of the shape.

Our framework is shown to be able to emulate several methods discussed in related literature and it enables a wide range of other beading schemes.
We show that we can define a scheme which reduces the amount of overfill and underfill compared to previous literature and we show that the extrusion widths have less deviation from the nozzle size.
We show that our open source implementation exhibits computation times in the same order of magnitude as the state-of-the-art implementation of the uniform width technique.
\end{abstract}
...
\begin{keyword} % SPM
Toolpath, 3D printing, adaptive width, underfill
\end{keyword}
