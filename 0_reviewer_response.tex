...
See \cref{intro_wedge_distributed} here. 
...
We have also changed the example shape of Fig. 16 (Now Fig. 17, reproduced here in \cref{visualized_accuracy}) to include a pointy wedge shape similar to Fig. 1.
}
\commits{
...
\Cref{no_corners} (of this document) shows an example of a shape without sharp corners, and which also requires a large bead width variation when using the constant bead count approach by Ding et al.
}
\begin{figure} \centering
...
We now address these facts in text and show statistics on the number of discontinuities in Fig. 18c.
...
\Ans{
The difference between the inward distributed beading scheme and the method by Jin et al 2017 JMS is already shown in Figs. 1c and 1b respectively. 
It is also shown in Figs. 17c and 17e (old Figs. 16c and 16e).
...
\Ans{
We have adjusted the range of the inward distributed scheme to $N=2$ in order to generate a bigger difference between these images (Now Fig. 17d and 17e).
...
The graph is now displayed in Fig. 18d.
...
\Ans{
The proposed technique relies \emph{less} on the accuracy of the control than existing techniques, seeing as the extrusion widths of the inward and evely distributed schemes exhibit less variation than the state-of-the-art centered technique.
See Fig. 18d (old Fig. 17b).
...
\paper{1. Introduction, bulletlist}{
...
We fixed a mistake when performing the statistical analysis. In the first submitted version we used a minimum bead width half of what it should have been for the centered scheme. The extrusion width statistics figure (now Fig. 18d, old Fig. 17b) now shows little beads below \SI{0.2}{\milli\meter} instead of below \SI{0.1}{\milli\meter}.
