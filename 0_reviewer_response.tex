...
\Ans{
There is not really a relationship between under-/overfill and the allowed variation in bead width,
aside from the microgaps generated at 3-way intersections which play a minute role.
The state-of-the-art (Jin et al 2017 JMS) does have a relationship: the ranges $[0,w_\text{min}]$ and $[w_\text{max},2w^*]$ produce underfill and overfill respectively, so making the allowed variation wider reduces the under-/overfill for their method and likewise for the centered beading scheme.
However, the distributed beading schemes have no such limitation.
They minimize under-/overfill with the requirement that the allowed variation in bead width is at least a factor of 2 ($[0.75w^*,1.5w^*]$).
\todo{So...?}

Because the beading scheme decides how to fill the full radial distance using some beading sequence, under-/overfill shouldn't be a problem anymore.
However, because there are certain interactions in 2D, this doesn't always hold.
For example a 3-way intersection in a 3D model which is a honeycomb pattern with a feature diameter of $w^*$ causes some over and underfill using \emph{any} possible technique.
Our proposed beading schemes efectively deal with over- and underfill, and because the current state-of-the-art did not, the proposed beading scheme indeed reduces the undefill/overfill areas.
}

\todo{
fix wording

Make changes in paper:
Clarify in intro, explain when introducing the Centered approach and show in the results section!
...
\Ans{
We now clearly indicate in the conclusion that we expect this paper to contribute to the reduction of Design for Aadditive Manufacturing (DfAM) constraints.
}
\todo{
Maybe we should add something like the following to the manuscript: (but where?)
}
