...
\item We have added a video animation explaining the union of cones approach in an intuitive manner.
\end{enumerate}
...
We can then derive that the naive scheme produces over-/underfill at all diameters except $d=2i w^*$, the centered scheme for $(i + 0.8) w^* < d < (i+ 1.25) w^*$ and the constant and distributed schemes nowhere (except for features smaller than $w^*$).
...
\paper{6.3 Limitations, first paragraph}{
\revise{}{
...
\paper{7. Conclusion, last paragraph}{
...
\parbox[b]{\figwidthTwo}{\subcaption{}\label{intro_wedge_uniform}}\includegraphics[width=\figwidth]{sources-intro-TEST-naive-accuracy}
\end{subfigure}
\begin{subfigure}{\figwidth}\centering
\parbox[b]{\figwidthTwo}{\subcaption{}\label{intro_wedge_centered}}\includegraphics[width=\figwidth]{sources-intro-TEST-Center-widths}
\end{subfigure}
\begin{subfigure}{\figwidth}\centering
\parbox[b]{\figwidthTwo}{\subcaption{}\label{intro_wedge_distributed}}\includegraphics[width=\figwidth]{sources-intro-TEST-Distributed-widths}
\end{subfigure}
\caption{
...
\includegraphics[width=\figwidthTwo]{sources-validation-wedge-TEST-Distributed-feature-size}
\caption{Widening meta-scheme using $w_\text{min} = 2 r_\text{min} = 0.1$
\\ on top of the Distributed scheme}
\label{min_feature_size}
\end{subfigure}
\begin{subfigure}[b]{\figwidthTwo}\centering
\includegraphics[width=\figwidthTwo]{sources-validation-wedge-TEST-Center-widening}
...
\paper{6 Results and discussion}
{
...
Nearly every model induces the full range of $[0.25w^*,1.8w^*]$ using the centered approach by \citeauthor{Jin2017JMS},
...
\begin{subfigure}[b]{\figwidth} \centering
\includegraphics[rotate=-90,width=\linewidth]{response_rects_Center_accuracy}
...
\paper{5.2 Print results}{\revise{}{The printing order is determined greedily by choosing the closest point of a polygonal extrusion path, or the closest of either end point in case of an open polyline extrusion path.}} 
\paper{6.1.3 Print time}
{
\revisepar{
...
\paper{6.2 Comparison of beading schemes}
{
...
\begin{subfigure}{\figwidth}\centering
\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-naive-accuracy}
\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-naive-widths}
\caption{Uniform}\label{TEST_naive_accuracy}
\end{subfigure}
%\begin{subfigure}{\figwidth}\centering
%\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-SingleBead-accuracy}
%\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-SingleBead-widths}
%\caption{Single}\label{TEST_SingleBead_accuracy}
%\end{subfigure}
\begin{subfigure}{\figwidth}\centering
\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-Constant-accuracy}
\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-Constant-widths}
\caption{Constant}\label{TEST_Constant_accuracy}
\end{subfigure}
\begin{subfigure}{\figwidth}\centering
\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-Center-accuracy}
\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-Center-widths}
\caption{Centered}\label{TEST_Center_accuracy}
\end{subfigure}
\begin{subfigure}{\figwidth}\centering
\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-Distributed-accuracy}
\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-Distributed-widths}
\caption{Distributed}\label{TEST_Distributed_accuracy}
\end{subfigure}
\begin{subfigure}{\figwidth}\centering
\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-InwardDistributed-accuracy}
\includegraphics[height=\figheight]{sources-validation-gMAT-example-TEST-InwardDistributed-widths}
...
\includegraphics[width=\figwidth]{sources-applications-P3-print-wedge-naive-edited}
\includegraphics[width=\figwidth]{sources-applications-P3-print-wedge-center-edited}
\includegraphics[width=\figwidth]{sources-applications-P3-print-wedge-inward-edited}
...
\begin{subfigure}{\figwidth}\centering
%\includegraphics[height=\figheightTwo]{sources-applications-gMAT-naive}
\includegraphics[height=\figheight]{sources-applications-P3-print-hex-naive-edited}
\includegraphics[width=\figwidth]{sources-applications-P3-print-UM-naive-edited}
\caption{Uniform}\label{print_naive}
\end{subfigure}
\begin{subfigure}{\figwidth}\centering
%\includegraphics[height=\figheightTwo]{sources-applications-gMAT-center}
\includegraphics[height=\figheight]{sources-applications-P3-print-hex-center-edited}
\includegraphics[width=\figwidth]{sources-applications-P3-print-UM-center-edited}
\caption{Centered}\label{print_center}
\end{subfigure}
\begin{subfigure}{\figwidth}\centering
%\includegraphics[height=\figheightTwo]{sources-applications-gMAT-inward}
\includegraphics[height=\figheight]{sources-applications-P3-print-hex-inward-edited}
\includegraphics[width=\figwidth]{sources-applications-P3-print-UM-inward-edited}
...
56bbbaf0b6de98f6e6d4b4fa849ee17cf98e4b3a % remove `for FDM'
}
\paper{Title}{
...
\paper{Abstract}
{
Furthermore, we propose a novel scheme\revise{ for FDM printing}{} which [\dots]
}
...
fc6e05c79340dafd15a9f95de175de696c427513 % explain contour vs direction parallel better
7a2b37513a940ea71a54be05075e46157084b40b % in related work
}
\paper{Abstract}{
...
\Ans{
The bead width in \citeauthor{Jin2017JMS} is limited to the interval $[0.25 w^*,1.8 w^*]$, whereas ours is limited to $[0.75 w^*, 1.5 w^*]$.

In the old manuscript we referred to the `deviation' from the preferred width as $1.8w^*/w^*=1.8$, wich was a confusing statistic.
Moreover, instead of the actual $1.8$ used by \citeauthor{Jin2017JMS} we used the theoretically maximum value of $2.0$ to arrive at a `deviation factor' of $2$.
We now don't refer to any such confusing statistic and instead just mention the range of widths itself.
}
\commits{
...
\paper{6.2 Comparison of beading schemes, paragraph 3, 4}{
In \cref{TEST_Center_accuracy} we can see that
the centered beading scheme effectively deals with \revise{both }{}overfill \revise{and underfill }{}and produces desired bead widths in all locations, except for the extrusion paths in the center, where the bead widths \revise{are within a factor 2 off from the desired bead width.}{range between $0.25 w^*$ and $1.8w^*$.}
...
\paper{6.3 Limitations, second paragraph}
{
\revisepar{}{
The default limit bisector angle $\alpha_\text{max} = \SI{135}{\degree}$ ensures that we don't employ transitioning in shallow wedge regions, which would result in a lot of short odd single bead polylines, which would break up the semi-continuous nature of polygonal extrusion paths;
$\alpha_\text{max} = \SI{135}{\degree}$ corresponds to $w^* / \cos \nicefrac12 \alpha_\text{max} \approx \SI{0.4}{\milli\meter}$ long segments
...
\paper{5.1 Back pressure compensation}
{
...
We have added a 3D animation showing the union of cones slicing technique.
\paper{3.1 Overview, third paragraph}
{
\revise{}{%afs
...
Furthermore:
\begin{itemize*}
\item Removed paragraph before bullet list of contributions, because it contained the same information as the bullet list.
\commits{0ed48668c93047be8423be054d98a81dfabce305}
\item Removed Future work section because the limitations are now handled in 6.3 Limitations.
\commits{e86c7b879a580f9fdcd599fcd9ff12a669f35190}
\item Removed wrapfigure explaining the area covered by a single extrusion move (in section~5.2.1~Accuracy), since that is already explained in the appendix.
\commits{d402ba3a47d35bc9a040b6ed4ccdfb85e911bedf}
...
\item Fixed double header of References.
\commits{7597b176f6a88ae98c8c7a4c791475af356b3da6}
\item Fixed various issues in References.
\commits{c524eae1a78dd1ca54293522139c44a68a911e80}
\item Reduced superfluous use of the term `we'.
\commits{a8f434957a577944ed16597de353349844a16462}
\item Improved syntac and flow of the text.
% numerous commits
