...
\subQue{
For example, what happens ifthe obtuse outside angle in Figure 2 goes to 0? 
}
...
Jins method is limited to something like $0.2-1.8 \times$ nozzle\_size
...
\Que{
It is unclear how some of the parameters are adjusted, such as $\alpha_{max}, d_\text{max}^\text{unmarked}, d_\text{transition}^\text{max}, t_\text{beading}, d_\text{intersection}^\text{max}$. What are their effects on the end result? Selections seem somewhat arbitrary. 
}
