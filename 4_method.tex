...
We make use of the union of cones~\cite{blum1967transformation} to visualize the skeleton, by raising each point in the domain to a height that equals the shortest distance from the point to the polygon boundary (Fig. 2?).
With the aid of this visualization, contour parallel toolpath with uniform width can be interpreted as the intersection of the union of cones with a set of planar planes at equally spaced heights (Fig. 2?).

As depicted in Fig. 2?-?, starting from the medial axis, our method first identifies edges and nodes in the skeleton that are located in the central of the polygon. The heights at these elements (the distance of the corresponding projected point to the polygon boundary) are then quantized in terms of the number of beads. To ensure a smooth toolpath between regions with quantized integer heights that different more than two, we add new nodes in the skeleton and assign them interpolated integer heights. The union of cones corresponding to the smoothed skeleton is then sliced at regular intervals to obtain a toolpath with varying width.
\jun{Do we use regular intervals, but get varying width?}

We note that the union of cones (or UoC) is introduced only for visualization. We describe how to construct the mesh of the UoC in \cref{sec_surface_construction} and
...
\jun{I would like to change the title, don't have a better word.}
...
It is defined by the locations where the inscribed circle meets the boundary in at least two locations~\cite{blum1967transformation}.
...
In our generalized framework the function $q$ is determined by a beading scheme.
