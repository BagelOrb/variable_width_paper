...
A novel way of conceptualizing the medial axis is as an efficient data structure to store a distance field.
The medial axis is the set of all points where the derivative of the distance field is discontinuous.

By introducing extra bones to the MAT, we can approximate insets from the medial axis by connecting points on the segments of the MAT using linear segments.

We introduce the Grounded Medial Axis Transform.
We dscretize the segments of the MAT where $R$ changes non-linearly (point-line parabolas and point-point bisectors) into short segments
and connect the introduced vertices to the two closest locations on the outline.
This allows us to directly generate accurate insets from the GMAT.

Note that by altering the outline by an infinitesimal change the straight skeleton of the polygon can be made to coincide with the GMAT.

We call any graph a GMAT if it contains the MAT and its segments are within the outline and the nodes have an accurate distance measure $R$ and the distance $R$ changes approximately linearly between two connected nodes.
The GMAT allows for extra segments.

We can generate an inset with rounded corners at distance $d$ by interpolating the $R$ at GMAT nodes and connecting the interpolated points.
Similarly one could compute an unmitered inset by connecting interpolated points along the straight skeleton.
...
\subsection{2. Significance measure}\label{sec:significance_measure}
...
Equivalently we can look at the angle $\alpha$ between a skeletal point and the two closest points on the outline.
(Same definition as \cite{attali1996modeling}.)
A computationally efficient way to compute the same can be obtained by looking at the ratio between $R$ and Euclidean distance.
...
\paragraph{line-point bones}
\hl{reformulate in terms of $\alpha$}
