...
Now that each node has been assigned a beading we proceed to generate the sites along its support edge which define the toolpath.
For each upward half-edge \jun{half-edge?} $\overrightarrow{v_0 v_1}$ we decide the location ($v$), width ($w$) of the $i$-th site $J$ based on the beading $B = B^{v_1}$:
\begin{align*}
J &= \{ v, w, i \} \\ 
