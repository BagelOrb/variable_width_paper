...
Given an arbitrarily shaped polygon which represents the intersection of a plane and a 3D shape, our method generates a toolpath with varying width, i.e. a set of (mostly) connected line segments, with each node being assigned a width. 
The width along a line segment is linearly interpolated from the widths of its two nodes.  

Our method starts with computing the skeleton of the input polygon by medial axis transform (MAT), a strategy that has been commonly used for generating contour parallel toolpath~\cite{eiamsa2003toward}. 
We make use of the union of cones to visualize the skeleton (\cref{sec_surface_construction}), by raising each point in the domain to a height that equals the shortest distance from the point to the polygon boundary (Fig. 2?).
Contour parallel toolpath with uniform width can be interpreted as the intersection of the union of cones with a set of planar planes at equally spaced heights (Fig. 2?).

As depicted in Fig. 2?-?, starting from the medial axis, our method first identifies edges and nodes in the skeleton that are located in the central of the polygon (\cref{sec_center_classification}). The heights at these elements are then quantized in terms of beads. To ensure a smooth toolpath between regions with quantized integer heights that different more than two, we add new nodes in the skeleton and assign them interpolated integer heights (\cref{sec_central_height_adjustment}). The union of cones corresponding to the smoothed skeleton is then sliced at regular intervals to obtain a toolpath with varying width (\cref{sec_toolpath_extraction}).
\jun{Do we use regular intervals, but get varying width?}
...
This section explains how we can generate toolpaths with evenly distributed bead widths. In \cref{sec_generalization} we describe how to generalize our method for generating evenly distributed bead widths into a general framework for generating contour parallel toolpaths using an arbitrary beading scheme and we show several such beading schemes.
...
\end{subfigure}
\caption{
The first row illustrates the generation of uniform paths (e) by interpreting the path as the intersection of horizontal planes and the union of cones (c) which is an alternative visualization of the skeleton (b). 
The second row depicts the major steps with both 2D and 3D visualizations for generating paths with adaptive width (i). Central elements in the skeleton are first identified (blue in f). The heights are then quantized in terms of beads (g, the integer values), and smoothed (h).
...
% and quantize the height in those regions \subref{3d_surface_overview_rounded}.
...
For ease of reference we have included a legend explaining \jun{this figure itself does not sufficiently "explain". How about indicating, highlighting etc?} the terms employed in this manuscript in \cref{legend}.
These terms will be further explained as they first appear later in the paper. 
...
Starting from the medial axis we further decompose the shape into simple fragments, so that the domain contains only quads and triangles. This decomposition allows to approximate the surface of the UoC as a 3D piecewise linear mesh. 


%\subsubsection{Medial axis transform}
\paragraph{Medial axis transform}
The medial axis is a compact and intrinsic representation of shapes.
It is defined by the locations where the inscribed circle meets the boundary in at least two locations~\cite{blum1967transformation,lee1982medial}.
...
We call the set of closest points on the outline polygon $P$ of a point $v$ on the skeleton its \emph{support}:
\begin{equation}
    \text{sup}(v) = \argmin_{x\in P} |x - v|.
\end{equation}
Consequently the shortest distance for a point on the skeleton is called its feature radius, $R(v)$.
...
%This last conceptualization is especially relevant because the uniform toolpath generation method consists precisely in equidistant wave-fronts from the outline.
...
By vertically raising the center of an inscribed circle to a height that equals the center's feature radius, a right cone is formed by the inscribed circle as the base and the raised center as the apex. 
The union of cones forms a 3D solid volume. 
The medial axis can thus also be interpreted as the ridges of the volume defined by the union of cones. 
...
To construct the union of cones, starting from the medial axis we decompose the input polygon into a set of quads and triangles.
\jun{Tim, what is our initial purpose of skeletal trapezoidation, the purpose before we concept the approach by union of cones?}
...
Besides the nodes that are connected by straight or parabolic edges, we insert nodes to discretize a parabolic edge into a polyline (see the middle of \cref{shape_decomposition_st}). 
The length of edges in the polyline is upper bounded by a prescribed constant $d^\text{discretization}$.
Additional nodes, with the same upper bound on the resulting edge length, are also added to discretize an edge that is formed by two concave outline vertices (see the vertex-vertex region in \cref{legend}). 
This allows to approximate the feature radius $R$ in between the two nodes $v_0$ and $v_1$ of an edge in the medial axis by interpolating linearly between $R(v_0)$ and $R(v_1)$.
...
%See the middle of \cref{shape_decomposition_st}.
...
We assign each node in ST a (floating) height value measured in terms of beads, referred to as the \emph{bead count}.
...
Note that although the overview of the method has been described geometrically in terms of the UoC, the actual method relies on the 2-dimensional ST, while the use of the bead count as a height value is only a visualization aid.
\jun{is it only a visualization or will it be used for slicing?}
...
We then assign each node the bead count values using Eq.~\ref{eq:initialBeadCount}.
We compute the Voronoi diagram using the Boost \verb!C++! libraries~\cite{schaling2011boost}, which implements the algorithm proposed by \citeauthor{fortune1986sascg}~\cite{fortune1986sascg}. A half-edge data-structure is used to represent the Voronoi diagram (\cref{shape_decomposition_datastructure}).

%

\jun{Since VD is not frequently used, I propose to suppress the use of VD. The full consequence needs to be checked.}
...
The bisector angle $\alpha$ is the interior angle $\angle{p_ovp_1} \leq \si{180}{\degree}$, formed by two support edges ($vp_0$ and $vp_1$) of node $v$~\cite{attali1996modeling}. 
\jun{Bisector angle is defined on nodes, but later we classify edges on either of its nodes, or?}
Edges are significant if the bisector angle exceeds $\alpha_\text{max}$ which is governed by the beading scheme. 
For a polygon with a pointy wedge area of an angle $\beta$, we have $\alpha = 180\degree - \beta$, which corresponds to overfill areas and underfill areas the size of $\nicefrac12 w^2 \left( \nicefrac14 \tan ( \alpha / 2) - \alpha / 2 \right)$ \jun{$w\to w_{pref}$ }when filled using the simple technique of uniform bead width $w$.
...
Whether the bisection angle is larger than a threshold $\alpha_\text{max}$ is equivalent to if $ | R(v_1) - R(v_0) | / |v_1 - v_0| >  \cos(\alpha_\text{max} / 2)$. See \cref{distance_based_angles}. This ratio has a clear geometrical interpretation as the slope of the ridge in the UoC surface.
...
See \cref{legend}.
\jun{This paragraph is not clear to me. Is it related to Fig. 6? what is $x$ and what is $d$?}
...
More specifically we round the floating heights ($b_\text{initial}$) to integer bead count values ($b_\text{quantized}$).
Because the bead count is defined in terms of the feature diameter rather than the radius, only half of the integer bead count values ($b_v/2$) coincides with a slicing height.
...
We define a quantization operator to map a feature diameter to a bead count: $q: \mathbb{R} \to \mathbb{N}$.
For examples in this section we choose to round to the nearest integer multiple of the nozzle size $w_\text{pref}$: $q(d) = \left\lfloor \nicefrac{d}{w_\text{pref}} + \nicefrac12 \right\rfloor$.
Alternative realizations of the function $q$ will be discussed in \cref{sec_generalization}.

We first assign each marked node $v$ of the ST the bead count $b^*_v \leftarrow q(2R(v))$.
