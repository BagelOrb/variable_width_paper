...
Because of \cref{saddle_points_are_marked} we only have to consider paths between marked regions which are monotonically in/decreasing in feature radius.
...
We also filter unmarked regions where the bead count remains constant.
\todo{Explaining why would have to refer forward to section about interpolating beadings}
...
In case $B^*_{v_0} = B^*_{v_1}$ we mark all bones and nodes $v_x$ in between, and set the bead count $B^*_{v_x} \leftarrow B^*_{v_0}$.

\todo{TODO: figure of basic bead counts left, middle with transition mids, right with transition ends.}
