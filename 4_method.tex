...
\paragraph{Beading} 
For toolpath geneWe associate each central node $v$ with a set of radial distances for each bead along with the width of that bead, which we call the \emph{beading} $B$.
For our distributed beading scheme we define the beading for a central node $v$ as:
\begin{align*}
    B &= \left\{ \left( w_0 , l_0 \right), \dots, \left( w_{n-1}, l_{n-1} \right) \right\} \\
    w_i &= 2R(v) / n  & \text{for all $i$}\\
    l_i &= 2R(v) / n (i + \nicefrac12) & \text{for all $i$}
\end{align*}
where
