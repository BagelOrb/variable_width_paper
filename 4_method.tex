...
We decide on a bead count at locations which lie in centeral positions of the shape in some sense to be defined below.
...
\item Generate junctions along unmarked bones of the skeleton and connect the junctions together to form the toolpaths.
\end{enumerate}
...
In marked regions of the VQ we employ the approach of applying the optimal bead counts and employing transitioning, while in unmarked regions the beading is only governed by the beading of locations with locally optimal feature radius.
...
At a distance $t$ from $v$ along marked bones we introduce an new node $v_0$ for the end of the transition and again we connect it to the outline as per the VQ requirements.
...
For any sequence on unmarked bones from the outline polygon inward we define the stable end point as the first vertex which is connected to a marked VQ segment.
...
In order to determine the junction locations of quads which don't contain any such node, we propagate the beading information down.
...
We then generate bead segments for each quad by connecting together the junctions of the same index.
If the amount of junctions on both sides of the quad is not the same then this quad is in a transition and we leave one junction unconnected.
...
These polylines generally form closed polygons, but at marked regions of the VQ the toolpaths consist of single bead polyline toolpaths.
