...
The naive method consists of consecutive inward offsets from the outline.

These offsets can be generated from the medial axis transform (MAT).
The medial axis transform can be conceptualized as the skeleton of the mesh of the volume defined by the union of all cones with \SI{45}{\degree} touching the input shape.
The constant width insets are then conceptualized as the intersection between the Union of Cones (UoC) and horizontal planes at regular intervals.

Alternatively: the MAT captures the internal part of the distance field and the constant width insets are the isocontours of the that distance field.

The problem with the constant width insets is that in the center of the shape \todo{it doesn't fit}.
because the distance to the boundary is not an integer multiple of the nozzle size.
We therefore change the mesh such that an exact integer beads will be generated.

A simplified version of our framework can be conceptualized as such:
\begin{itemize}
...
\item Smooth out sharp transitions between regions with different integer multiple slice heights
...
\begin{figure}
\centering
\setlength{\figwidth}{.3\columnwidth}
\begin{subfigure}{\figwidth}
\includegraphics[width=\columnwidth]{sources/method/surface/simple_surface_mat.png}
\includegraphics[width=\columnwidth]{sources/method/surface/naive.pdf}
\caption{Union of Cones}
\end{subfigure}
\begin{subfigure}{\figwidth}
\includegraphics[width=\columnwidth]{sources/method/surface/simple_surface_no_transitions.png}
\includegraphics[width=\columnwidth]{sources/method/surface/no_transitions.pdf}
\caption{Rounded}
\end{subfigure}
\begin{subfigure}{\figwidth}
\includegraphics[width=\columnwidth]{sources/method/surface/simple_surface_st.png}
\includegraphics[width=\columnwidth]{sources/method/surface/distributed.pdf}
\caption{Smoothed}
\end{subfigure}
\caption{
A simplified geometrical conceptualization of our framework.
The 3D surface representations (top) are sliced into the toolpaths (bottom) which fill the outline shape.
We first compute the Union of Cones mesh, then alter it to prevent shallow slopes and then smooth it to prevent instantaneous changes.
}
...
Not all non-outline nodes are central.
How to determine the height of such nodes?

\Cref{section_toolpath_extraction} shows how we can determine the heights of non-central verts, extends the simplified method to a generalized framework and provides an efficient algorithm to slice the modified UoC.
...
We decide on a bead count at locations which lie in central positions of the shape and apply an optimal distribution of widths from the central regions outward.
...
\subsection{Center classification}
\todo{TODO: explain section title!}
...
\subsection{Central height adjustment}
\todo{TODO: explain section title!}
...
\subsection{Peripheral height adjustment}
