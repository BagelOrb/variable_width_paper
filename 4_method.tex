...
Our method starts with computing {a graph which represents the topology of the input polygon: its \emph{skeleton}. 
...
Contour-parallel toolpath with uniform width can be interpreted as the intersection of the union of cones with a set of {horizontal} planes at equally spaced heights (\cref{overview_outline,overview_skeleton,overview_uoc,overview_uniform_sliced,overview_uniform_paths}).
...
The medial axis is a {representation commonly used to analyze} a shape.
...
\end{enumerate*}
The vertex-line and vertex-vertex edges are discretized into pieces with a length up to {\SI{0.2}{\milli\meter}, which gives an approximation error of only $\pm$\SI{0.01}{\milli\meter}}.
...
In order to prevent over- and underfill {from occurring} in central{ regions}, we mark parts of the ST as being central.
...
{A too large $\alpha_\text{max}$ may leave a lot of under-/overfill, while a too small value may introduce toolpaths to fill in negligibly small underfills.
...
{We use $d_\text{max}^\text{unmarked} = w^*$ in order to filter out high frequency oscillations in the order of magnitude of the nozzle size, while keeping close to significance measure.}
...
{A value of $d_\text{max}^\text{transition} = \SI{1}{\milli\meter}$ seems to produce satisfactory results.}
{This means that for some small regions we generate toolpaths with bead widths outside the typical range.}
...
{The transition length $t(n)$ ensures that the center beads don't overlap with the innermost transitioning beads, while keeping the amount of underfill low and the toolpath smooth.
...
Now that we have determined the bead counts in the {marked }central regions we will describe how the {unmarked} regions are handled.
...
and interpolate between $v_m$ and the beading at the top of the slope over some distance $t_\text{beading}$ from the lower marked node{, which we set to $t_\text{beading} = w^* $, so that the transition is not too swift.}
...
{The} downward phase makes sure that all nodes have a beading associated with it, so that the slicing algorithm can efficiently slice the edges leading up to a marked or unmarked node.	
