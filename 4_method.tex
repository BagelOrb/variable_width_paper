...
\subref{shape_decomposition_domains} The ST divides the shape into primitives which can be classified as belonging to one of the inout shape polygons.
}
...
The bisector angle $\alpha$ is the interior angle $\angle{p_ovp_1} \leq \si{180}{\degree}$ between a location $v$ on a bone of the ST and its two supporting polygon points $p_0$ and $p_1$.\cite{attali1996modeling}
...
We mark all significant bones as such and contrary to related literature we keep the unmarked bones of the ST.
...
This means we can prevent executing trigonometric functions on \emph{each} of the bones in the ST.
Moreover the ratio has a clear geometrical interpretation as the slope of the ridge in the UoC surface.

This ratio can readily be applied to simple ST segments to determine whether they are significant.
For segments generated by a polygon vertex at $(0,d)$ and a line segment through $(0,0)$ and $(0,\epsilon)$ or another vertex $(0,0)$, we can determine the significant portion by evaluating $\frac{\partial R}{\partial x} > \cos(\alpha_\text{max} / 2)$.
...
We first assign each marked node $v$ of the ST the bead count $b^*_v \leftarrow f(2R(v))$.
...
We require an additional function to be specified by the beading strategy: $t_0(n)$ determines the distance between the lower end of a transition from bead count $n$ to $n+1$ and the anchor point - as measured along the marked bones of the ST.
...
The linear interpolation enforces robustness against small perturbations in the outline shape which cause extra bones to appear in the ST inside of transition regions.
