...
Given arbitrarily shaped polygons which represent the 2D outline of a layer of a 3D model, our method generates a toolpath with varying width, i.e. a set of (mostly) connected line segments, with each site being assigned a location and an extrusion width;
...
In order to retreat a polyline ending in a site $J$ we remove part of the polyline paths up to the intersection by a distance of $w^J d_\text{max}^\text{intersection}$, where $d_\text{max}^\text{intersection}$ is some ration between $0$ and $1$.
