...
From any node in the graph which is not a saddle point there is only a single edge going upward.
\todo{Explanation / proof needed.}
%See VoronoiQuadrangulation::filterUnmarkedRegions
This means that walking up the graph from a marked region to a local maximum follows a single path without branchings.
