...
More specifically we round the floating heights ($b^\sim$) to integer bead count values ($b^\neg$).
Because the bead count is defined in terms of the feature diameter rather than the radius, only half of the integer bead count values ($b^\sim/2$) coincides with a slicing height.
...
For a marked edge which connects nodes $v_0$ and $v_1$ with $b^\neg_{v_0}\le n<b^\neg_{v_1}$, we determine \emph{transition anchors} at which the bead count transits from $n$ to $n+1$.
...
Therefore, we modify nodes that are within $t(n)/2$ by (re-)assigning them a fractional bead count $b^\wedge$, linearly interpolated from the ratio of the distance over $t(n)$ (see \cref{beading_transitioning_filtering__transitions_applied}). 
...
If a central edge has both nodes with an odd bead count assigned to it $b^\wedge \bmod 2 = 1 $ we should prevent the algorithm from generating the center extrusion segment from both trapezoids.
