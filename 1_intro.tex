...
\citeauthor{Ding2016a} developed a toolpath strategy for wire and arc additive manufacturing which produces a width variation typically lower than a factor of $3$, but is  far greater{ for some parts}~\cite{Ding2016a,Xiong2019}.
...
The current {state of the art} for FDM printing developed by \citeauthor{Jin2017JMS} employs a strategy which alters the widths of the centermost beads {within a range of widths with a factor of $3.6$}~\cite{Jin2017JMS},
% (0.8 + 1) / (2* (1.25 - 1)), see rmin, rmax, Table 1e, Centered strategy
...
{and introduce a beading scheme which reduces the bead width variation compared to the state of the art.} 
...
\end{subfigure}
\caption{
Illustration of different toolpath{s} for a shape {showcasing a range of shape radii }{(black)}.
{These results can be read as a graph with feature size on the horizontal axis and its corresponding beading along the vertical axis.}
...
\subref{intro_wedge_distributed} Our approach minimizes over- and underfill with {less extreme widths}.
}
\label{intro_wedge}
\end{figure}
...
\begin{itemize}
\item A geometric framework {allowing various adaptive bead width control schemes used to generate contour-parallel toolpaths which minimize under- and overfill.}
\item A specific beading scheme  which
{furthermore} reduces the amount of {variation} in the extrusion widths compared to {the state of the art to be below a factor of 2.}
