...
\paragraph{Uniform Beading Strategy}
We can define a beading strategy which emulates the uniform width offsetting technique by disabling the marking of edges, so that we never employ transitioning.
\begin{align*}
...
We therefore base the beading strategy on the bead count $q^-(d)$ defined by the uniform beading strategy.
...
Moreover, the bead widths equal the nominal bead width for large regions meaning that mechanical properties derived for prints using the uniform strategy will still hold approximately.
