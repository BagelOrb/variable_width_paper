...
\subsection{Comparison of beading schemess}
...
The 'Constant bead count' scheme effectively deals with underfills, but generates orders of magnitude more overfills compared to the other schemess. 
...
However, according to \cref{widthHistogram} the Centered scheme exhibits a lot wider range of bead widths than the Distributed schemess:
the standard deviation of the bead widths in the Centered scheme is approximately \SI{39}{\micro\meter}, while the standard deviation resulting from the Distributed schemess is approximately \SI{14}{\micro\meter}.
We can therefore state that the Distributed schemess result in bead widths closer to the nominal widths compared to the Centered scheme.
...
In the beading schemess we discussed we set the length of the smoothed transitions to $t(n) = w_\text{pref}$.
...
Our schemess require a more accurate control of the volumetric flow rate (in \si{\milli\meter\cubed\per\second}).
...
Rather than deriving some optimal beading scheme from a clear specification of the objective, we propose some arbitrary inward distributed beading scheme and show that it is better than the other beading schemess.
