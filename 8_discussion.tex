...
We therefore state that the distributed schemes result in bead widths closer to the preferred widths compared to the centered scheme.
...
However, when comparing the bead width of the outer toolpaths, \cref{distributed_comparison} and  \cref{widthIndexedHistogram}, we see that the these toolpaths have the preferred bead width more often, which means that errors from inaccuracies due to deviation from the preferred bead width are concentrated near the center of the shape, leaving the dimensional accuracy of the outline shape less affected, if at all. 
